...
import {
  idlFactory as daoAnalyticsIdl,
  // eslint-disable-next-line @typescript-eslint/ban-ts-comment
  // @ts-ignore - _SERVICE is generated by dfx at build time
  _SERVICE as DAOAnalyticsService,
} from "@declarations/dao_analytics/dao_analytics.did.js";
import {
...
export const initializeAgents = async (identity?: Identity) => {
  ...
  try {
    ...
    const dao_analytics = await createActor<DAOAnalyticsService>(
      import.meta.env.VITE_CANISTER_ID_DAO_ANALYTICS,
      daoAnalyticsIdl,
      identity
    );

    // Ledger (optional in dev, required for real transfers)
    const ledger = await (async () => {
      const id = (import.meta as any).env.VITE_CANISTER_ID_LEDGER as string | undefined;
      if (!id) {
        if (import.meta.env.DEV) return createMockActor<LedgerService>();
        throw new Error("Missing environment variable: VITE_CANISTER_ID_LEDGER");
      }
      try {
        return await createActor<LedgerService>(id, ledgerIdl, identity);
      } catch (e) {
        if (import.meta.env.DEV) return createMockActor<LedgerService>();
        throw e;
      }
